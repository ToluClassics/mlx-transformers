...
class BertEmbeddings(nn.Module):
    ...
    def __call__(
        self, input_ids: mx.array, position_ids: mx.array, token_type_ids: mx.array = None
    ) -> mx.array:
        words = self.word_embeddings(input_ids)

        position_ids = mx.arange(input_ids.shape[1]) if position_ids is None else position_ids
        ...
        print(self.LayerNorm(embeddings))
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x: mx.array) -> mx.array:
        ...
        return x.reshape(bsz, seq_len, self.num_attention_heads, self.attention_head_size).transpose(
            0, 2, 1, 3
        )
