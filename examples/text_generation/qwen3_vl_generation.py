...
def load_model(
    model_name: str,
    args,
) -> Tuple[MlxQwen3VLForConditionalGeneration, AutoProcessor]:
    config = AutoConfig.from_pretrained(model_name)

    model = MlxQwen3VLForConditionalGeneration(config)
...
def generate(
    model: MlxQwen3VLForConditionalGeneration,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwen3-VL inference script")
    parser.add_argument(
        "--model-name",
